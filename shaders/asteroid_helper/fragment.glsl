...
layout(std140,binding = 3) uniform Steroid{
	Blob balls[128];
	int ball_count;
	float threshold ;
	int octaves;
...
bool in_cube(vec3 pos){
	vec3 a_pos = abs(pos);
	return max(a_pos.x,max(a_pos.y,a_pos.z)) <= 5.0001;
}

vec2 nearestIntersection(){
	float planes[6] = 
		{(5 - v_c_pos.x)/ v_c_dir.x,
	(-5 - v_c_pos.x)/ v_c_dir.x,
	(5 - v_c_pos.y)/ v_c_dir.y,
	(-5 - v_c_pos.y)/ v_c_dir.y,
	(5 - v_c_pos.z)/ v_c_dir.z,
	(-5 - v_c_pos.z)/ v_c_dir.z};

	vec2 extremes = vec2(9999,-9999);

	for(int i = 0;i<6;i++){
		if(in_cube(v_c_pos + planes[i] * v_c_dir)){
			extremes = vec2(min(planes[i], extremes.x),max(planes[i], extremes.y));
		}
	}
...
// Mix final noise value
  ...
  return vec4(g0+g1+g2+g3, 105.0 * dot( m4, v_grads ));
}
...
vec4 evaluate(vec3 pos){
	...
	for(int i = 0;i<ball_count;i++){
		...
		result += fbm(pos, fractal) * 0.01;// * brightness + base_brightness;
	}
...
void main(){
	vec2 intersection = nearestIntersection();
	vec3 pos = v_c_pos + v_c_dir * intersection.x;
	int steps = iterations;

	vec3 color = vec3(0);
	vec3 delta = v_c_dir * ((intersection.y - intersection.x) / (steps-1));
	vec3 normal = vec3(0);
	for(int i = 0;i<steps;i++){
		vec4 value = evaluate(pos);
		if(value.w > threshold){
			color = vec3(1) * max(0.1,dot(normalize(value.xyz), vec3(0,1,0)));
