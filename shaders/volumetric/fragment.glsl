...
layout(std140,binding=4) uniform StarPoints{
...
float light_transmittance(vec3 position, vec3 light, int steps){
	...
	for(int i = 0;i<steps;i++){
		transmittance += max(0,fbm(pos)) * step * density;
		pos += delta;
	}
...
vec3 cast_ray(vec3 start, vec3 end, int steps){
	...
	for(int i = 0;i<steps;i++){
		...
		for(int l = 0;l<star_count;l++){
			float distance = min(1 / dot((pos - star_positions[l]), (pos- star_positions[l])),5) * scattering(dot(normalize(-delta), normalize(pos - star_positions[l])));
			color += vec3(scatt) * distance * max(0,fbm(pos)) *  exp(-(transmittance + light_transmittance(pos, star_positions[l],steps)));
		}
		pos += delta;
	}
	return color;
...
void main(){
	...
	vec3 direction = vPos - camera;
	vec2 intersection;
	if(find_intersection(vPos, direction, radius, intersection) ){
		vec3 color = cast_ray(vPos + max(0,intersection.x) * direction, vPos + direction * intersection.y, iterations);
		fCol = vec4(color,1);
	}
	else{
		discard;
	...
	//vec3 gritty = floor(vPos * 10);
	//fCol = vec4(vec3(mod((gritty.x + gritty.y + gritty.z),2) * 0.2 + 0.3),1);
	//fCol = vec4(vec3(vPos) * 0.5 + 0.5,1);
}
