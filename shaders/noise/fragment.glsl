...
float noise(vec3 position){
	...
	return (value * brightness) + baseline;
}
...
vec3 displace(vec3 pos){
	return vec3(fbm(pos + vec3(0,0,displacement)),
...
void main(){
	f_col = vec4(vec3(fbm(v_pos) * brightness + 0.5),1);
	//f_col = vec4(vec3(dot(normalize(v_norm), vec3(0,0.8,0))),1);
	//f_col = vec4(v_norm,1);
}
